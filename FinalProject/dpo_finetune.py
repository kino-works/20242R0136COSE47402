...
# 손실 함수
def dpo_loss(logits, temperature=1.0):
    pos_score, neg_score = logits[:, 0], logits[:, 1]
    return -torch.log(torch.exp(pos_score / temperature) / (torch.exp(pos_score / temperature) + torch.exp(neg_score / temperature))).mean()

# 데이터로더 생성
processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch32")
dpo_dataset = PreferenceDataset(train_captions, processor, os.path.join(data_dir, "flickr30k_images"), mode="DPO")
dpo_dataloader = DataLoader(
    dpo_dataset, batch_size=512, shuffle=True, collate_fn=collate_fn
)
...
for epoch in range(num_epochs):
    model.train()
    total_loss = 0
    progress = tqdm(dpo_dataloader, desc=f"Epoch {epoch + 1}/{num_epochs} (DPO)")
    ...
    # 모델 저장
    output_dir = f"./clip_dpo_finetune/clip_finetuning_dpo_{epoch}"
